...
class ChatStore:
    ...
    async def save_pipeline_state(self, chat_id: str, state: dict[str, Any]) -> None:
        ...
        for key in ("diagnosis", "triage_result", "body_part", "detections", "patient_info", "mongo_patient_id", "report_url"):
    ...
    async def get_pipeline_state(self, chat_id: str) -> dict[str, Any]:
        ...
        row = await mongo_service.db.chat_sessions.find_one(
            ...
            projection={
                ...
                "pipeline_report_url": 1,
            },
        )
        if not row:
            return {}
        out: dict[str, Any] = {}
        for key in ("diagnosis", "triage_result", "body_part", "detections", "patient_info", "pending_report_actor_role", "mongo_patient_id", "report_url"):
